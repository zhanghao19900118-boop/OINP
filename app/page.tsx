...
export default function Home() {
  const [view, setView] = useState<"home"|"type"|"quiz"|"result"|"admin">("home");
  ...
  useEffect(()=>{ try { const saved=localStorage.getItem("oinp-state"); if(saved){const s=JSON.parse(saved);setAnswers(s.answers||{});setStep(s.step||0);setDoctor(!!s.doctor)} setWechat(localStorage.getItem("oinp-wechat")||"OINP-Consult");setQr(localStorage.getItem("oinp-qr")||""); } catch {} },[]);
  ...
  return <main>
    <header><button className="brand" onClick={()=>setView("home")} aria-label="返回首页"><span className="trillium">✦</span><span>Ontario EOI <small>评分工具</small></span></button><div className="header-right"><span className="policy">政策版本 · 2026.07</span><button className="admin-link" onClick={()=>setView("admin")}>管理</button></div></header>
