...
复制 `.env.production.example` 为 `.env.production` 并设置随机密钥、管理员账号和密码。客户数据默认保存到 `OINP_DATA_DIR` 指定的目录，请将该目录纳入服务器备份。

管理员登录地址为 `/?admin=1`，公开页面不会显示管理入口。
...
将本项目全部文件上传到空白仓库即可。请勿上传 `node_modules`、`.sites-runtime`、`dist` 和 `.next`。

## 当前版本说明

- 用户密码以加盐哈希保存，会话使用 HttpOnly Cookie。
- NOC 搜索为内置样例，后续可接入完整职业数据库或后端 API。
- 评分规则属于信息辅助工具，上线前应继续依据安省官方最新规则复核。
